...
pub fn input() -> (Vec<u8>, Vec<[[u8; BOARD_SIZE]; BOARD_SIZE]>) {
    ...
    for board_lines in lines.chunks(BOARD_SIZE) {
        println!("{:?}", board_lines);
        let mut board: [[u8; BOARD_SIZE]; BOARD_SIZE] = [[0; BOARD_SIZE]; BOARD_SIZE];
        for i in 0..BOARD_SIZE {
            ...
            let nums: Vec<u8> = line
                ...
                .map(|x| {
                    println!("x = {}", x);
                    x.parse::<u8>().expect(&format!("unable to parse num from {}", x))
                })
                .clone()
                .collect();
...
pub fn part1(moves: Vec<u8>, mut boards: Vec<[[u8; BOARD_SIZE]; BOARD_SIZE]>) -> (u32, u32) {
    ...
    'outer: for num in moves {
        for board_idx in 0..boards.len() {
            let mut board = boards[board_idx];
...
fn board_sum(board: [[u8; BOARD_SIZE]; BOARD_SIZE]) -> u32 {
    let x: u32 = board
        .iter()
